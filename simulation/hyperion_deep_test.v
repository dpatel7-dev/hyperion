// ─────────────────────────────────────────
// HYPERION DEEP TEST
// 2 stacked neural network layers
// ─────────────────────────────────────────

module hyperion_deep_test;
    ...
    always @(posedge clk) begin
        if (layer1_done && !last_l1_done)
            $display("  Layer 1 complete — output feeding into layer 2...");
        if (layer2_done && !last_l2_done) begin
            $display("  Layer 2 complete!");
            $display("");
            $display("Final output (2 layers deep):");
            ...
            $display("");
            $display("══════════════════════════════════════════");
            $display("Hyperion DEEP — 2 stacked layers complete.");
            $display("Input → Layer1 → Layer2 → Output");
            $display("512 MAC operations total.");
            $display("GPT has 96 of these. Hyperion has 2.");
            $display("Same architecture. Different scale.");
            $display("══════════════════════════════════════════");
            $finish;
        end
        last_l1_done <= layer1_done;
        last_l2_done <= layer2_done;
    end

    initial begin
        clk=0; reset=1; start=0;
        last_l1_done=0; last_l2_done=0;
        ...
        $display("Hyperion DEEP — 2 stacked layer test");
        $display("══════════════════════════════════════════");
        $display("Input → Layer1(×W1+B1→ReLU) →");
        $display("        Layer2(×W2+B2→ReLU) → Output");
        $display("══════════════════════════════════════════");
