...
run_sim_tests() {
    ...
    run_test "Embedding table" \
        "iverilog -o /tmp/t_emb verilog/embedding_table.v simulation/embedding_test.v && vvp /tmp/t_emb" \
        "Hyperion embedding table working"
    ...
    run_test "LoRA unit" \
        "iverilog -o /tmp/t_lora verilog/lora_unit.v simulation/lora_test.v && vvp /tmp/t_lora" \
        "Hyperion LoRA unit working"
...
echo ""
echo -e "  ${CYAN}[1]${NC}  ${WHITE}All tests${NC}             ${DIM}29 tests — simulation + synthesis${NC}"
echo -e "  ${CYAN}[2]${NC}  ${WHITE}Simulation only${NC}       ${DIM}16 tests — verify correct output${NC}"
echo -e "  ${CYAN}[3]${NC}  ${WHITE}Synthesis only${NC}        ${DIM}13 tests — verify gate counts${NC}"
...
case $CHOICE in
    1) run_sim_tests; run_synth_tests ;;
    2) run_sim_tests ;;
    3) run_synth_tests ;;
    4)
        echo ""
